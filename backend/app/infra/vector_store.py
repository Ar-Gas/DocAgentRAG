...
def reset_clients() -> None:
    global _chroma_client, _chroma_collection, _chroma_block_collection
    with _client_lock:
        _chroma_client = None
        _chroma_collection = None
        _chroma_block_collection = None
    ...
    with _client_lock:
        ...
        try:
            ...
            if collection_exists:
                try:
                    collection.add(documents=["维度测试文档"], ids=["dimension_test_temp"])
                    collection.delete(ids=["dimension_test_temp"])
                    logger.info("现有collection维度匹配")
                except Exception as exc:
                    if "dimension" in str(exc).lower():
                        logger.warning("检测到维度不匹配，删除旧collection: %s", exc)
                        client.delete_collection(name="documents")
                        collection_exists = False
                    else:
                        logger.warning("测试collection时出错: %s", exc)
            if not collection_exists:
                collection = client.create_collection(name="documents", embedding_function=doubao_ef)
            ...
            try:
                ...
                logger.info("Chroma客户端初始化成功（使用本地BGE模型: %s）", bge_model)
                _chroma_client = client
                _chroma_collection = collection
                return client, collection
                ...
                try:
                    ...
                    try:
                        client, collection = init_ephemeral_chroma_client()
                        _chroma_client = client
                        _chroma_collection = collection
                        return client, collection
...
def get_block_collection():
    ...
    with _client_lock:
        ...
        try:
            ...
            logger.warning("初始化 document_blocks collection 失败，使用默认配置重试: %s", exc)
